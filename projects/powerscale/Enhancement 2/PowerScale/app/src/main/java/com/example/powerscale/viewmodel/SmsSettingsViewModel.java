 ...
 *
 * This class:
 * - Loads the saved SMS settings state
 * - Validates the phone number input
 * - Updates SMS enabled preference
...
public class SmsSettingsViewModel extends AndroidViewModel {
    ...
    public SmsSettingsViewModel(@NonNull Application application) {
        ...
        // Initialize repository used for preference operations
    ...
    /**
     * loadSmsSettings()
     * Loads the saved SMS settings and emits the current UI state.
     */
    public void loadSmsSettings() {
        boolean hasPermission = hasSmsPermission();
        boolean enabledPref = repository.isSmsEnabledPreference();
        String phone = repository.getSavedPhone().trim();

        // If permission is not granted, force the stored enabled state off
        if (!hasPermission && enabledPref) {
            repository.setSmsEnabledPreference(false);
            enabledPref = false;
        }
    ...
    public void handleToggleChanged(boolean wantsEnabled) {
        ...
        if (wantsEnabled) {
            ...
            repository.setSmsEnabledPreference(true);
     ...
     * Validates and stores the SMS phone number.
     *
    ...
    public void savePhone(String phoneText) {
        ...
        if (phone.isEmpty()) {
            smsResult.setValue(new Event<>(buildStateResult(enabledPref, hasPermission, repository.getSavedPhone().trim(), null, getApplication().getString(R.string.phone_required), OperationStatus.ERROR)));
            return;
        }
    ...
    public void sendTestSms(String phoneText) {
        ...
        if (!enabledPref) {
            smsResult.setValue(new Event<>(buildStateResult(false, hasPermission, repository.getSavedPhone().trim(), getApplication().getString(R.string.sms_enable_first), null, OperationStatus.ERROR)));
            return;
        }
        ...
        if (!hasPermission) {
            ...
            smsResult.setValue(new Event<>(buildStateResult(false, false, repository.getSavedPhone().trim(), getApplication().getString(R.string.sms_permission_required_toggle), null, OperationStatus.ERROR)));
            return;
        }

        if (phone.isEmpty()) {
            smsResult.setValue(new Event<>(buildStateResult(true, true, repository.getSavedPhone().trim(), null, getApplication().getString(R.string.phone_required), OperationStatus.ERROR)));
            return;
        }
     ...
     *
     * @param enabledPref - True if SMS is enabled in preferences.
